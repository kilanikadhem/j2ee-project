...
<body>
	<c:if test="${requestScope.listeArticles == null}">
		<c:redirect url="/modifier" />
	</c:if>

	<jsp:include page="modifierformulaire.jsp" />
	<jsp:include page="modifierformulairecat.jsp" />
	...
	<div class="row" id="bodym">
					...
					<div class="form-group">
						...
						<div class="col-sm-9">
							<select id="cat" class="form-control"
								...
								<option value="modifier">Tout les produits</option>
								<c:forEach var="cat" items="${applicationScope['categories']}">
									<option value="modifier?catid=${cat.cat_id}">${cat.cat_nom}</option>
								</c:forEach>
							</select>
						</div>
					</div>
						...
						<div class="col-lg-2">
							<div class="thumbnail">
								...
								<div class="caption">
									...
									<a href="modifierArticle?a_id=${article.a_id}"
		...
		<div class="col-lg-2" id="contenu">
			<div class="page-header">
				...
				<c:if test="${requestScope.testModificationArticle == true}">
					<p class="bg-success">Categorie modifié avec succée !</p>
				</c:if>
				<c:if test="${requestScope.testModificationCategorie == false}">
					<p>Cette categorie est liée a des articles !</p>
				</c:if>
			</div>
			<div class="list-group">
				<c:forEach var="cat" items="${applicationScope['categories']}">
					<a class="list-group-item" href="modifierCategorie?cat_id=${cat.cat_id}" class="list-group-item">${cat.cat_nom}</a>
				</c:forEach>
			</div>
		</div>
	</div>
